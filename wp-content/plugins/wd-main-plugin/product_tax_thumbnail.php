...
if(!class_exists('Showcase_Taxonomy_Images')) {
	class Showcase_Taxonomy_Images {
		...
		public function init () {
			// Image actions
			add_action('product_provider_add_form_fields', array($this, 'add_category_image'), 10, 2);
			add_action('created_product_provider', array($this, 'save_category_image'), 10, 2);
			add_action('product_provider_edit_form_fields', array($this, 'update_category_image'), 10, 2);
			add_action('edited_product_provider', array($this, 'updated_category_image'), 10, 2);
			add_action('admin_enqueue_scripts', array($this, 'load_media'));
			add_action('admin_footer', array($this, 'add_script'));
		}

		public function load_media () {
			if(!isset($_GET['taxonomy']) || $_GET['taxonomy'] != 'product_provider') {
				return;
			}
			wp_enqueue_media();
		}
        ...
        <label for="showcase-taxonomy-image-id"><?php _e('Image', 'ajzaa'); ?></label>
        ...
        <p>
          <input type="button" class="button button-secondary showcase_tax_media_button" id="showcase_tax_media_button"
                 name="showcase_tax_media_button" value="<?php _e('Add Image', 'ajzaa'); ?>"/>
          <input type="button" class="button button-secondary showcase_tax_media_remove" id="showcase_tax_media_remove"
                 name="showcase_tax_media_remove" value="<?php _e('Remove Image', 'ajzaa'); ?>"/>
        </p>
      </div>
		<?php }
        ...
        <td>
          ...
          <p>
            ...
            <input type="button" class="button button-secondary showcase_tax_media_remove"
                   ...
                   value="<?php _e('Remove Image', 'ajzaa'); ?>"/>
          </p>
        </td>
      </tr>
		<?php }
